Public Class frmImagCTScan
    Dim index As Integer
    Dim errmsg As String

    Private Sub frmImagCTScan_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    'To initialise the form details
    Public Sub initialisationLoad()
        cmbPatID.DropDownHeight = cmbPatID.ItemHeight * 3
        cmbfname.DropDownHeight = cmbfname.ItemHeight * 3

        Dim y, y1 As Integer
        Dim max, max2, max3 As Integer

        'To retrieve test relation with department
        sqlstr1 = "SELECT Test_ID, Dept_ID FROM Test_Details WHERE Test_Name='CT Scan'"
        ...
        sqlstr = "SELECT B.Pat_ID,B.Doc_ID,C.Doc_FName,A.Pat_FName,A.Pat_Age,A.Pat_Gender FROM Patient_Info A, Diagnosis_Tests B ,Doctor_Info C WHERE B.Pat_ID=A.Pat_ID AND C.Doc_ID=B.Doc_ID AND B.CTScanTest='Yes' AND B.CT_Test_Status='Pending'"
    ...
    Private Sub insertNewTestRecords()
        ...
        If saveDialog() = True Then
            Try
                cmd.CommandText = "INSERT INTO CTScan_Tests(CTScanTest_ID,Dept_ID,Test_ID,Pat_ID,Ref_DocID,Ref_RadioID,CTScanTest_Date,CTScanTest_Name,Remark_1,Remark_2,Remark_3,Remark_4,Remark_5,Remark_6,Opinion)VALUES(@CTScanTest_ID,@Dept_ID,@Test_ID,@Pat_ID,@Ref_DocID,@Ref_RadioID,@CTScanTest_Date,@CTScanTest_Name,@Remark_1,@Remark_2,@Remark_3,@Remark_4,@Remark_5,@Remark_6,@Opinion)"

                cmd.Parameters.AddWithValue("@CTScanTest_ID", Val(txtctscantest.Text))
                ...
                If res > 0 Then
                    ...
                    Try
                        cmd1.CommandText = "UPDATE Diagnosis_Tests SET Pat_ID=@Pat_ID,CT_Test_Status=@CT_Test_Status WHERE Pat_ID=@Pat_ID"
                        cmd1.Parameters.AddWithValue("@Pat_ID", Val(cmbPatID.Text))
                        cmd1.Parameters.AddWithValue("@CT_Test_Status", "Done")
    ...
    Private Sub incrCTScanTest()
        Dim x As Integer
        Dim max2 As Integer
        sqlstr2 = "SELECT CTScanTest_ID FROM CTScan_Tests"
